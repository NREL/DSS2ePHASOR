...
# Scan each line to find the number of unique "New"s for the number of different line configurationsr
...
#            yr41=tokens1[6][1].split('-')[1]            
        ...
        for h in range(0,len(tokens1)):
            if tokens1[h][0] =='xhl' and tokens1[h][0] =='buses':
                for h in range(0,len(tokens1)):
...
f=open("regulator.dss", "r")
fileString = f.read()
f.close()
processedString = re.sub("\n\s*\n*", "\n", fileString)
f=open("regulator.dss", "w")
f.write(processedString)
f.close()

# Scan lines of both "Lines.dss" and "Services.dss"
with open("regulator.dss") as f2:
    lines1=f2.readlines()
    count=len(lines1)
