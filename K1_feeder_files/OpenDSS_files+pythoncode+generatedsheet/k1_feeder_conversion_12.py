...
for n in range(0,len(s2)):
    ...
    for j  in range(0,nu):
        if tokens1[j][0]== 'units':
            tokens1[j][1]=(tokens1[j][1]) 
...
bus1_a = [None] * len(Linesd)
bus1_b = [None] * len(Linesd)
bus1_c = [None] * len(Linesd)
for y in range(0,q3_1):
    if 'switch' not in Linesd[y].keys():
        bus1_a[y]=Linesd[y].get('bus1')
        bus1_a[y]=bus1_a[y]+'_a'
        bus1_a[y]=bus1_a[y].lower()
        
        bus1_b[y]=Linesd[y].get('bus1')
        bus1_b[y]=bus1_b[y]+'_b'
        bus1_b[y]=bus1_b[y].lower()

        bus1_c[y]=Linesd[y].get('bus1')
        bus1_c[y]=bus1_c[y]+'_c'
        bus1_c[y]=bus1_c[y].lower()
        
bus2_a = [None] * len(Linesd)
bus2_b = [None] * len(Linesd)
bus2_c = [None] * len(Linesd)

for y in range(0,q3_1):
    if 'switch' not in Linesd[y].keys():
        bus2_a[y]=Linesd[y].get('bus2')
        bus2_a[y]=bus2_a[y]+'_a'
        bus2_a[y]=bus2_a[y].lower()
        
        bus2_b[y]=Linesd[y].get('bus2')
        bus2_b[y]=bus2_b[y]+'_b'
        bus2_b[y]=bus2_b[y].lower()

        bus2_c[y]=Linesd[y].get('bus2')
        bus2_c[y]=bus2_c[y]+'_c'
        bus2_c[y]=bus2_c[y].lower()
        
linename = [None] * len(Linesd)
for y in range(0,q3_1):
    if 'switch' not in Linesd[y].keys():
        linename[y]='line_'+Linesd[y].get('linenames')
        linename[y]=linename[y].lower()
        
linelength = [None] * len(Linesd)
for y in range(0,q3_1):
    if 'switch' not in Linesd[y].keys():
        ...
        if units == "km":
            convf = km2mil
        linelength[y]=Linesd[y].get('length')*convf
...
for y in range(0,q3_1):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if LineCodesd[y1].get('units') == "km":
            convfactor = km2mil
            if R_0 in LineCodesd[y1].values():
                temp=(LineCodesd[y1].get('r0'))/convfactor
        if LineCodesd[y1].get('units') == "m":
            convfactor = mil2mil
            if R_0 in LineCodesd[y1].values():
                temp=(LineCodesd[y1].get('r0'))/convfactor
        if LineCodesd[y1].get('units') == "kft":
            convfactor = kf2mil
            if R_0 in LineCodesd[y1].values():
                temp=(LineCodesd[y1].get('r0'))/convfactor
    r_0[y]=temp
    ...
    for y1 in range(0,len(LineCodesd)):
        if LineCodesd[y1].get('units') == "km":
            ...
            if X_0 in LineCodesd[y1].values():
                temp=(LineCodesd[y1].get('x0'))/convfactor
        if LineCodesd[y1].get('units') == "m":
            convfactor = mil2mil
            if X_0 in LineCodesd[y1].values():
                temp=(LineCodesd[y1].get('x0'))/convfactor
...
for y in range(0,q3_1):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_1 in LineCodesd[y1].values():
            temp=(LineCodesd[y1].get('r1'))/convfactor
    r_1[y]=temp
...
for y in range(0,q3_1):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_1 in LineCodesd[y1].values():
            temp=(LineCodesd[y1].get('x1'))/convfactor
    x_1[y]=temp
...
for y in range(0,q3_1):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_0 in LineCodesd[y1].values():
            temp=(LineCodesd[y1].get('c0'))*(2*3.14*60/(1000*convfactor))
    c_0[y]=temp
...
for y in range(0,q3_1):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_1 in LineCodesd[y1].values():
            temp=(LineCodesd[y1].get('c1'))*(2*3.14*60/(1000*convfactor))
    c_1[y]=temp
    ...
    if tokens1[3][0] !='switch':
        for h in range(0,len(tokens1)):
            ...
            if tokens1[h][0] =='bus2':
                tr2=tokens1[h][1]
                yr2=tr2.split('.')[0]
            if tokens1[h][0] =='length':
                yr3=tokens1[h][1]
            if tokens1[h][0] =='linecode':
                yr4=tokens1[h][1]
            if tokens1[h][0] =='units':
                yr5=tokens1[h][1] 
                hr[n]=yr1+' '+yr2+' '+yr3+' '+yr4+' '+yr5

    for h in range(0,len(tokens1)):
        if tokens1[h][0] =='switch': 
            hr[n]=''
...
#y+q3_1 makes sure the service line data are added after the OH and UG line information
for y in range(0,len(cleanlist1)):
    bus1_a[y+q3_1]=services_list[y][0]
    bus1_a[y+q3_1]=bus1_a[y+q3_1]+'_a'
    bus1_a[y+q3_1]=bus1_a[y+q3_1].lower()
    
    bus1_b[y+q3_1]=services_list[y][0]
    bus1_b[y+q3_1]=bus1_b[y+q3_1]+'_b'
    bus1_b[y+q3_1]=bus1_b[y+q3_1].lower()

    bus1_c[y+q3_1]=services_list[y][0]
    bus1_c[y+q3_1]=bus1_c[y+q3_1]+'_c'
    bus1_c[y+q3_1]=bus1_c[y+q3_1].lower()

for y in range(0,len(cleanlist1)):
    bus2_a[y+q3_1]=services_list[y][1]
    bus2_a[y+q3_1]=bus2_a[y+q3_1]+'_a'
    bus2_a[y+q3_1]=bus2_a[y+q3_1].lower()

    bus2_b[y+q3_1]=services_list[y][1]
    bus2_b[y+q3_1]=bus2_b[y+q3_1]+'_b'
    bus2_b[y+q3_1]=bus2_b[y+q3_1].lower()

    bus2_c[y+q3_1]=services_list[y][1]
    bus2_c[y+q3_1]=bus2_c[y+q3_1]+'_c'
    bus2_c[y+q3_1]=bus2_c[y+q3_1].lower()
    
for y in range(0,len(cleanlist1)):
    linename[y+q3_1]='line_'+services_list[y][1]
    linename[y+q3_1]=linename[y+q3_1].lower()

for y in range(0,len(cleanlist1)):
    units=services_list[y][4]
    if units == "kft":
        convf = kf2mil
    if units == "ft":
        convf = f2mil
...
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if LineCodesd[y1].get('units') == "m":
            convfactor = m2mil
        if LineCodesd[y1].get('units') == "mi":
            convfactor = mil2mil
        if LineCodesd[y1].get('units') == "kft":
            convfactor = kf2mil
        if LineCodesd[y1].get('units') == "ft":
            convfactor = f2mil 
        if R_11temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,0])/convfactor
    R_11[y+q3_1]=temp

X_11=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_11temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,0])/convfactor
    X_11[y+q3_1]=temp

C_11=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_11temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,0])/(convfactor*2.6526)
    C_11[y+q3_1]=temp

R_21=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_21temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,1])/convfactor
    R_21[y+q3_1]=temp

X_21=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_21temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,1])/convfactor
    X_21[y+q3_1]=temp

C_21=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_21temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,1])/(convfactor*2.65)
    C_21[y+q3_1]=temp

R_22=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_22temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,2])/convfactor
    R_22[y+q3_1]=temp

X_22=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_22temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,2])/convfactor
    X_22[y+q3_1]=temp        

C_22=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_22temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,2])/(convfactor*2.65)
    C_22[y+q3_1]=temp

R_31=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_31temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,3])/convfactor
    R_31[y+q3_1]=temp

X_31=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_31temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,3])/convfactor
    X_31[y+q3_1]=temp        

C_31=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_31temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,3])/(convfactor*2.65)
    C_31[y+q3_1]=temp

R_32=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_32temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,4])/convfactor
    R_32[y+q3_1]=temp

X_32=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_32temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,4])/convfactor
    X_32[y+q3_1]=temp

C_32=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_32temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,4])/(convfactor*2.65)
    C_32[y+q3_1]=temp

R_33=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if R_33temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['rmatrix'][0,5])/convfactor
    R_33[y+q3_1]=temp

X_33=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if X_33temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['xmatrix'][0,5])/convfactor
    X_33[y+q3_1]=temp

C_33=[None]*len(Linesd)
for y in range(0,len(cleanlist1)):
    ...
    for y1 in range(0,len(LineCodesd)):
        ...
        if C_33temp in LineCodesd[y1].values():
            temp=(LineCodesd[y1]['cmatrix'][0,5])/(convfactor*2.65)
    C_33[y+q3_1]=temp
...
for ty in range(0,fr):
    ...
    if linenames1 == "line":
        ...
        tokens1 = [x.replace(" ", "").split('=') for x in tokens1]
        tokens1 = { x:y for x,y in tokens1}
        tokens1['linenames'] = linenames15
        tokens4[counts]=tokens1
        counts=counts+1
...
#for ty in range(0,counts):
#    tokens4[ty]=dict(tokens1)     
...
for y in range(0,len(tokens4)):
    if 'switch' in tokens4[y].keys():        
        bus1_s[0+3*yst]=tokens4[y].get('bus1')
        if "." in bus1_s[0+3*yst]:
            bus1_s[0+3*yst]=bus1_s[0+3*yst].split('.')[0]
            bus1_s[0+3*yst]=bus1_s[0+3*yst]+'_a'
            bus1_s[0+3*yst]=(bus1_s[0+3*yst]).lower()
        else:
            bus1_s[0+3*yst]=bus1_s[0+3*yst]+'_a'
        ...
        bus1_s[1+3*yst]=tokens4[y].get('bus1')
        if "." in bus1_s[1+3*yst]:
            bus1_s[1+3*yst]=bus1_s[1+3*yst].split('.')[0]
            bus1_s[1+3*yst]=bus1_s[1+3*yst]+'_b'
            bus1_s[1+3*yst]=(bus1_s[1+3*yst]).lower()
        else:
            bus1_s[1+3*yst]=bus1_s[1+3*yst]+'_b'
        ...
        bus1_s[2+3*yst]=tokens4[y].get('bus1')
        if "." in bus1_s[2+3*yst]:
            bus1_s[2+3*yst]=bus1_s[2+3*yst].split('.')[0]
            bus1_s[2+3*yst]=bus1_s[2+3*yst]+'_c'
            bus1_s[2+3*yst]=(bus1_s[2+3*yst]).lower()
        else:
            bus1_s[2+3*yst]=bus1_s[2+3*yst]+'_c'
...
for y in range(0,len(tokens4)):
    if 'switch' in tokens4[y].keys():
        bus2_s[0+3*yst]=tokens4[y].get('bus2')
        if "." in bus2_s[0+3*yst]:
            bus2_s[0+3*yst]=bus2_s[0+3*yst].split('.')[0]
            bus2_s[0+3*yst]=bus2_s[0+3*yst]+'_a'
            bus2_s[0+3*yst]=(bus2_s[0+3*yst]).lower()
        else:
            bus2_s[0+3*yst]=bus2_s[0+3*yst]+'_a'
        ...
        bus2_s[1+3*yst]=tokens4[y].get('bus2')       
        if "." in bus2_s[1+3*yst]:
            bus2_s[1+3*yst]=bus2_s[1+3*yst].split('.')[0]
            bus2_s[1+3*yst]=bus2_s[1+3*yst]+'_b'
            bus2_s[1+3*yst]=(bus2_s[1+3*yst]).lower()
        else:
            bus2_s[1+3*yst]=bus2_s[1+3*yst]+'_b'
        ...
        bus2_s[2+3*yst]=tokens4[y].get('bus2')
        if "." in bus2_s[2+3*yst]:
            bus2_s[2+3*yst]=bus2_s[2+3*yst].split('.')[0]
            bus2_s[2+3*yst]=bus2_s[2+3*yst]+'_c'
            bus2_s[2+3*yst]=(bus2_s[2+3*yst]).lower()
        else:
            bus2_s[2+3*yst]=bus2_s[2+3*yst]+'_c'
...
for y in range(0,len(Linesd)):
    if 'switch' in Linesd[y].keys():
        line_sw[0+3*yst]='sw_'+Linesd[y].get('linenames')
        line_sw[0+3*yst]=line_sw[0+3*yst]+'_a'
        line_sw[0+3*yst]=(line_sw[0+3*yst]).lower()
        
        line_sw[1+3*yst]='sw_'+Linesd[y].get('linenames')
        line_sw[1+3*yst]=line_sw[1+3*yst]+'_b'
        line_sw[1+3*yst]=(line_sw[1+3*yst]).lower()
        
        line_sw[2+3*yst]='sw_'+Linesd[y].get('linenames')
        line_sw[2+3*yst]=line_sw[2+3*yst]+'_c'
        line_sw[2+3*yst]=(line_sw[2+3*yst]).lower()        
        yst=yst+1
for y in range(0,len(tokens4)):
    if 'switch' in tokens4[y].keys():  
        line_sw[0+3*yst]='sw_'+tokens4[y].get('linenames')
        line_sw[0+3*yst]=line_sw[0+3*yst]+'_a'
        line_sw[0+3*yst]=(line_sw[0+3*yst]).lower()
        
        line_sw[1+3*yst]='sw_'+tokens4[y].get('linenames')
        line_sw[1+3*yst]=line_sw[1+3*yst]+'_b'
        line_sw[1+3*yst]=(line_sw[1+3*yst]).lower()
        
        line_sw[2+3*yst]='sw_'+tokens4[y].get('linenames')
        line_sw[2+3*yst]=line_sw[2+3*yst]+'_c'
        line_sw[2+3*yst]=(line_sw[2+3*yst]).lower()
...
for y in range(0,len(tokens4)):       
    if 'switch' in tokens4[y].keys():
        status_sw[0+3*yst]=tokens4[y].get('enabled')
        if status_sw[0+3*yst] == 'Yes' or status_sw[0+3*yst] =='yes' or status_sw[0+3*yst] =='true':
            status_sw[0+3*yst]=1
        elif status_sw[0+3*yst] == 'No' or status_sw[0+3*yst] =='no' or status_sw[0+3*yst] =='false':
            status_sw[0+3*yst]=0
        else:
            status_sw[0+3*yst]=1
            
        status_sw[1+3*yst]=tokens4[y].get('enabled')
        if status_sw[1+3*yst] == 'Yes' or status_sw[1+3*yst] =='yes' or status_sw[1+3*yst] =='true':
            status_sw[1+3*yst]=1
        elif status_sw[1+3*yst] == 'No' or status_sw[1+3*yst] =='no' or status_sw[1+3*yst] =='false':
            status_sw[1+3*yst]=0
        else:
            status_sw[1+3*yst]=1
            
        status_sw[2+3*yst]=tokens4[y].get('enabled')
        if status_sw[2+3*yst] == 'Yes' or status_sw[2+3*yst] =='yes' or status_sw[2+3*yst] =='true':
            status_sw[2+3*yst]=1
        elif status_sw[2+3*yst] == 'No' or status_sw[2+3*yst] =='no' or status_sw[2+3*yst] =='false':
            status_sw[2+3*yst]=0
        else:
            status_sw[2+3*yst]=1
        yst=yst+1
...
#                yr1=tokens1[h][1]
            ...
            hr[j]='Bus='+yr1+' '+'Phase='+yr2+' '+'kv='+str(yr3)+' '+'kw='+str(yr4)+' '+'kvar='+str(yr5)
            qw[j]=yr1 
    tokens2[j] = re.findall(r'[\w\.\_]+[\ ]*=[\ ]*[\w\.\_]+', hr[j])
...
for k in range(0,len(cleanlist1)):
    ...
    toke2[k] = re.findall(r'[\w\.\_]+[\ ]*=[\ ]*[\w\.\_]+', Busn[k])
...
for ty in range(0,fr):
    ...
    if linenames1 == "Transformer":
        ...
        qw2=yr1+'*'+yr4
        hr='Busl='+qw2+' '+'FrombuskV='+yr2+' '+'FrombuskVA='+yr3+' '+'Bus2='+yr4+' '+'TobuskV='+yr5+' '+'TobuskVA='+yr6+' '+'%r='+yr7+' '+'%x='+yr8+' '+'Frombuswdg='+yr9+' '+'Tobuswdg='+yr10
...
for k in range(0,fr):
    ...
    if linenames1 == "Capacitor":
        ...
        for j in range(0,len(tokens1)):
            if tokens1[j][0]== "bus":
                if "." in tokens1[0][1]:
                   yr1=tokens1[j][1] .split(".")[0]
                   phnum=tokens1[j][1] .split(".")[1]
                else:
                    yr1=tokens1[j][1]
                    phnum=0
        for j in range(0,len(tokens1)):
        ...
        hr[k]='Bus='+yr1+' '+'kV='+yr2+' '+'kVAr='+yr3+' '+'conn='+yr4+' '+'phnum='+str(phnum)   
...
for y in range(0,len(cap1)):
    if cap1[y].get('phnum') == '0':
        ...
        P_c[y]=0
        Q_c[y]=-1*temp
    if cap1[y].get('phnum') == '1':
        temp=(cap1[y].get('kVAr'))
        P_a[y]=0
        Q_a[y]=-1*temp
        P_b[y]=0
        Q_b[y]=0
...
# Take OH and UG lines "To Bus" information for all the bus information

df=pd.read_csv("EPRIK1_EXP_VOLTAGES.CSV")
df.shape
...
for ty in range(0,fr):
    ...
    if linenames1 == "Transformer":
        ...
        for h in range(0,len(tokens1)):
            if tokens1[h][0] =='buses':
                sd1=tokens1[h][1]
                bus1=sd1.split(",")[0].split("(")[1].split(".")[0]
                bus2=sd1.split(",")[1].split(")")[0].split(".")[0]

        for h in range(0,len(tokens1)):
            if tokens1[h][0] =='kvs' or tokens1[h][0] =='kVs':
                sd2=tokens1[h][1]
                kv1=sd2.split(",")[0].split("(")[1]
                kv2=sd2.split(",")[1].split(")")[0]
...
if os.path.isfile('ExistingPV.txt'):
    # Existing PV
    f=open("ExistingPV.txt", "r")
    fileString = f.read()
    f.close()
    processedString = re.sub("\n\s*\n*", "\n", fileString)
    f=open("ExistingPV.txt", "w")
    f.write(processedString)
    f.close()

    with open("ExistingPV.txt") as f1:
    ...
    q2=0
    p=0
    f=open("distributedgen.txt", "r")
    fileString = f.read()
    f.close()
    processedString = re.sub("\n\s*\n*", "\n", fileString)
    f=open("distributedgen.txt", "w")
    f.write(processedString)
    f.close()

    with open("distributedgen.txt") as f1:
        lines22=f1.readlines()
        count=len(lines22)
    ...
    ## Current injector dataframe creation
    df8 = pd.DataFrame()
    df8 = df8.append({'Bus': '' ,'ID':''}, ignore_index=True)
    df8 = df8[['Bus','ID'  ]]
    ##

###############################################
### Bus Initialization spreadsheet creation ###
###############################################
...
## Creating individual lists
ID_load = [None] * len(Load2)*6
for k in range(0,len(Load2)):
    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6]=temp
    ID_load[k*6]=(ID_load[k*6]).lower()    
    ID_load[k*6]=(ID_load[k*6])+'/P_a'  

    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6+1]=temp
    ID_load[k*6+1]=(ID_load[k*6+1]).lower()    
    ID_load[k*6+1]=(ID_load[k*6+1])+'/Q_a'  

    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6+2]=temp
    ID_load[k*6+2]=(ID_load[k*6+2]).lower()    
    ID_load[k*6+2]=(ID_load[k*6+2])+'/P_b'  

    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6+3]=temp
    ID_load[k*6+3]=(ID_load[k*6+3]).lower()    
    ID_load[k*6+3]=(ID_load[k*6+3])+'/Q_b'   

    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6+4]=temp
    ID_load[k*6+4]=(ID_load[k*6+4]).lower()    
    ID_load[k*6+4]=(ID_load[k*6+4])+'/P_c'  

    temp='Load_'+(Load2[k].get('Busn'))
    ID_load[k*6+5]=temp
    ID_load[k*6+5]=(ID_load[k*6+5]).lower()    
    ID_load[k*6+5]=(ID_load[k*6+5])+'/Q_c' 
...
for i in range(0,1):
    ...
    temp1=temp.split("*")[0]
    temp2=temp.split("*")[1]
    ID_tf[i*3]="VR_"+temp1+"_"+temp2
    ID_tf[i*3]=(ID_tf[i*3]).lower()
    ID_tf[i*3]=(ID_tf[i*3])+'/tap_a'    

    temp=(Trans3.get('Busl'))
    temp1=temp.split("*")[0]
    temp2=temp.split("*")[1]
    ID_tf[i*3+1]="VR_"+temp1+"_"+temp2
    ID_tf[i*3+1]=(ID_tf[i*3+1]).lower()
    ID_tf[i*3+1]=(ID_tf[i*3+1])+'/tap_b'   

    temp=(Trans3.get('Busl'))
    temp1=temp.split("*")[0]
    temp2=temp.split("*")[1]
    ID_tf[i*3+2]="VR_"+temp1+"_"+temp2
    ID_tf[i*3+2]=(ID_tf[i*3+2]).lower()
    ID_tf[i*3+2]=(ID_tf[i*3+2])+'/tap_c'   
...
writer = pd.ExcelWriter("test_2_new.xlsx", engine='xlsxwriter')
